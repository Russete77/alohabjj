...
export class Renderer {
  ...
  private getEnvPalette(agent: Agent): CharacterPalette {
    ...
    switch (this.env) {
      ...
      case 'dojo': {
        ...
        return {
          ...agent.palette,
          shirt: giBlue ? '#3E63A8' : '#F0F0F2',
          pants: giBlue ? '#34558F' : '#E4E4E8',
          shoes: agent.palette.skin,
        };
      }
      default:
  ...
  private drawAgent(agent: Agent): void {
    ...
    // ── Faixa de BJJ (graduação) — agent.team carrega a cor da faixa ──
    const beltColor = agent.team && agent.team.startsWith('#') ? agent.team : null;
    if (beltColor) {
      const row = 9; // cintura no grid 12x16 do sprite
      ctx.fillStyle = beltColor;
      ctx.fillRect(cx + 2 * scale, cy + row * scale, 8 * scale, scale * 1.1);       // faixa
      ctx.fillRect(cx + 5 * scale, cy + row * scale, scale * 2, scale * 2);         // nó
      ctx.fillRect(cx + 5 * scale, cy + (row + 2) * scale, scale, scale * 1.6);     // ponta 1
      ctx.fillRect(cx + 6.2 * scale, cy + (row + 2) * scale, scale, scale * 1.6);   // ponta 2
      // graus (faixa preta): 2 riscos brancos no nó
      if (beltColor === '#111111') {
        ctx.fillStyle = '#F5F5F5';
        ctx.fillRect(cx + 5.2 * scale, cy + row * scale, scale * 0.4, scale * 2);
        ctx.fillRect(cx + 6.1 * scale, cy + row * scale, scale * 0.4, scale * 2);
      }
    }
